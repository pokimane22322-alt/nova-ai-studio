...
function LogoModel({ scrollProgress }: { scrollProgress: number }) {
  ...
  useFrame((state) => {
    ...
    groupRef.current.rotation.x = Math.sin(state.clock.elapsedTime * 0.25) * 0.03 * settle;
    groupRef.current.rotation.z = Math.sin(state.clock.elapsedTime * 0.2) * 0.02 * settle;

    meshRefs.current.forEach((mesh, i) => {
      if (!mesh || !blocks[i]) return;
      const b = blocks[i];
      mesh.position.lerpVectors(b.center, b.scatterPos, eased);
      mesh.position.y += Math.sin(state.clock.elapsedTime * 0.8 + i) * 0.015 * settle;
